...
import { retrieveEnvVariable, retrieveTokenValueByAddress, handleSlotChange} from './utils';
...
async function init(): Promise<void> {
  ...
  switch (QUOTE_MINT) {
    ...
    default: {
      throw new Error(`Unsupported quote mint "${QUOTE_MINT}". Only support SOL swap in Freemium version`);
    }
  }
  ...
  logger.info(`Take profit percent: ${TAKE_PROFIT}`)
  logger.info(`Stop loss percent: ${STOP_LOSS}`)
  logger.info(`Start sniping...`)
